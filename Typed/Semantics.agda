...
data _⟼_ : ∀ {τ} -> CTerm τ -> CTerm τ -> Set where
  ...
  Return : ∀ {Δ τ l} {Γ : Env Δ} {t : Term Δ τ} ->
             (Γ , Return t) ⟼ (Γ , Mac t)

  Dist->>= : ∀ {Δ l α β} {Γ : Env Δ} {c : Term Δ (Mac l α)} {k : Term Δ (α => Mac l β)} ->
              (Γ , c >>= k) ⟼ ((Γ , c) >>= (Γ , k))
  ...
  Bind : ∀ {Δ l α β} {Γ : Env Δ} {t : Term Δ α} {k : CTerm (α => Mac l β)} ->
           ((Γ , Mac t) >>= k) ⟼ (k $ Γ , t)

  BindEx : ∀ {Δ l α} {Γ : Env Δ} {e : Term Δ Exception} {k : CTerm (Exception => Mac l α)} ->
           ((Γ , Macₓ e) >>= k) ⟼ (Γ , Throw e)  -- Rethrown as in LIO. It could be also (Γ , Macₓ e)

  Throw : ∀ {Δ} {l : Label} {α : Ty} {Γ : Env Δ} {e : Term Δ Exception} -> (Γ , Throw e) ⟼ (Γ , Macₓ e)

  Dist-Catch : ∀ {Δ} {l : Label} {α : Ty} {Γ : Env Δ} {m : Term Δ (Mac l α)} {h : Term Δ (Exception => Mac l α)} -> 
  ...
  Catch : ∀ {Δ} {l : Label} {α : Ty} {Γ : Env Δ} {t : Term Δ α} {h : CTerm (Exception => Mac l α)} -> 
            Catch (Γ , Mac t) h ⟼ (Γ , (Return t))

  CatchEx : ∀ {Δ} {l : Label} {α : Ty} {Γ : Env Δ} {e : Term Δ Exception} {h : CTerm (Exception => Mac l α)} -> 
            Catch (Γ , Macₓ e) h ⟼ (h $ Γ , e)

  label : ∀ {Δ l h α} {Γ : Env Δ} {t : Term Δ α} -> (p : l ⊑ h) -> 
            (Γ , label p t) ⟼ (Γ , Return (Res t))

  Dist-unlabel : ∀ {Δ α l h} {Γ : Env Δ} {p : l ⊑ h} {t : Term Δ (Labeled l α)} -> 
                 (Γ , unlabel p t) ⟼ unlabel p (Γ , t)

  unlabel : ∀ {Δ l h α} {Γ : Env Δ} {p : l ⊑ h} {t : Term Δ α} ->
            unlabel p (Γ , Res t) ⟼ (Γ , Return t)

  unlabelCtx : ∀ {α l h} {p : l ⊑ h} {c c' : CTerm (Labeled l (Mac h α))} -> c ⟼ c' ->
